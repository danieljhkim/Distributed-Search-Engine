...
public class GlobalExceptionInterceptor implements ServerInterceptor {
    ...
    private Status mapExceptionToStatus(Throwable t) {
        ...
        if (t instanceof ShardNotFoundException e) {
            return Status.NOT_FOUND.withDescription("Shard not found: " + e.getShardId());
            ...
            return Status.FAILED_PRECONDITION.withDescription("Index initialization failed: " + e.getMessage());

        } else if (t instanceof IndexOperationException e) {
            // I/O or write failures – could also choose UNAVAILABLE
            return Status.INTERNAL.withDescription("Index operation failed: " + e.getMessage());
        }
