...
public class SearchController {
    ...
    @Timed(
            value = "dsearch.search.http",
            extraTags = {"endpoint", "/api/v1/search"})
    ...
    public SearchResponseDto search(@Valid @RequestBody SearchRequestDto req) {
        ...
        try {
            ...
            sample.stop(Timer.builder("dsearch.gateway.search.latency")
                    .tag(
                            "searchType",
                            req.getSearchType() != null ? req.getSearchType().name() : "UNKNOWN")
                    .tag("partitionId", req.getPartitionId() != null ? req.getPartitionId() : "UNKNOWN")
                    .tag(
                            "fusionStrategy",
                            req.getFusionStrategy() != null
                                    ? req.getFusionStrategy().name()
                                    : "UNKNOWN")
