...
public final class HybridFusion {
     ...
     * Main fusion method.
     *
     * @param bm25Result     lexical (BM25) result
     * @param semanticResult dense/semantic result
     * @param strategy       fusion strategy (SCORE_SUM, WEIGHTED, RRF, ...)
     * @param limit          max number of fused hits to return
     * @param alpha          weight for BM25 (for WEIGHTED)
     * @param beta           weight for semantic (for WEIGHTED)
     * @param rrfK           RRF parameter k
     */
    ...
    public static List<SearchHit> fuse(
        ...
        List<SearchHit> bm25Hits = bm25Result != null && bm25Result.getHits() != null
                ? bm25Result.getHits()
                : List.of();
        List<SearchHit> semHits = semanticResult != null && semanticResult.getHits() != null
                ? semanticResult.getHits()
                : List.of();
    ...
    private static Map<String, RankedScore> toRankedMap(List<SearchHit> hits) {
        ...
        for (int i = 0; i < hits.size(); i++) {
            ...
            map.put(h.getDocId(), new RankedScore(
                    h.getScore(),
                    i,
                    h.getTitle(),
                    h.getContent()
