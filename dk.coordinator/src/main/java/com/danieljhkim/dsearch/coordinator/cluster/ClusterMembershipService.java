...
public class ClusterMembershipService {
	...
	public ClusterMembershipService(AppConfig appConfig) {
		...
		this.coordinatorGroup = NodeGroup.fromConfig(NodeRole.NODE_ROLE_COORDINATOR,
				appConfig.getCoordinatorNodes());
	}
	...
	public void updateNodeHealth(String nodeId, NodeRole role, boolean isHealthy) {
		...
		if (existingNode != null) {
			NodeInfo updatedNode = new NodeInfo(existingNode.nodeId(), existingNode.host(),
					existingNode.port(), existingNode.healthPort(), existingNode.role(), isHealthy);
	 ...
	 */
	public record NodeInfo(String nodeId, String host, int port, int healthPort, String role,
			boolean isHealthy) {
		public NodeInfo {
			Objects.requireNonNull(nodeId, "nodeId must not be null");
	...
	public static class NodeGroup {
		...
		public static NodeGroup fromConfig(NodeRole role, AppConfig.NodeGroupConfig config) {
			...
			if (config.getNodes() != null) {
				for (AppConfig.NodeConfig nodeConfig : config.getNodes()) {
					NodeInfo nodeInfo = new NodeInfo(nodeConfig.getId(), nodeConfig.getHost(),
							nodeConfig.getPort(), nodeConfig.getHealthPort(), role.name(), true);
		...
		public AppConfig.NodeGroupConfig toNodeGroupConfigClone() {
			...
			List<AppConfig.NodeConfig> nodeConfigs = getAllNodes().stream().map(nodeInfo -> {
